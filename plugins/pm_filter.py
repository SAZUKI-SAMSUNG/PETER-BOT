...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("➪ 𝐇𝐄𝐘 𝐄𝐄 𝐂𝐈𝐍𝐈𝐌𝐀 𝐄𝐕𝐈𝐃𝐄𝐘𝐈𝐋𝐋𝐀.", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
...
async def advantage_spoll_choker(bot, query):
    ...
    if k==False:
        ...
        if files:
            ...
            k = await query.message.edit('ഈ സിനിമ ഇറങ്ങിയില്ല അല്ലേൽ എന്റ സൈറ്റിൽ കിട്ടിയില്ല എന്നോട് നിങ്ങൾ സെമിക്കുക💌')
    ...
    if query.data.startswith("file"):
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                ...
                await query.answer('𝙷𝙴𝚈 {query.from_user.first_name} 𝙵𝙸𝙻𝙴 𝙸𝚂 𝚈𝙾𝚄𝚁 𝙿𝙼 𝙸 𝙷𝙰𝚅𝙴 𝚂𝙴𝙽𝚃 𝚈𝙾𝚄𝚁 𝙿𝙼',show_alert = True)
        ...
        buttons = [[
            ...
            InlineKeyboardButton('𝙴𝚇𝚃𝚁𝙰 𝙼𝙾𝙳𝙴𝚂', callback_data='extra')
            ],[
            InlineKeyboardButton('𝙷𝙾𝙼𝙴', callback_data='start'),
            InlineKeyboardButton('𝙵𝙸𝙻𝙼𝚂', callback_data='stats')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        buttons= [[
            InlineKeyboardButton('𝚂𝙾𝚄𝚁𝙲𝙴', url='https://t.me/NOKKIERUNNOEPPPKITTUM')
            ],[
            InlineKeyboardButton('𝙷𝙾𝙼𝙴', callback_data='start'),
...
async def advantage_spell_chok(msg):
    ...
    await msg.reply("ഇതിൽ ഇത് സിനിമയാണ് വേണ്ടത് താഴെ കൊടുത്തിട്ടുണ്ട്👇 ഞാൻ.📌", reply_markup=InlineKeyboardMarkup(btn))
